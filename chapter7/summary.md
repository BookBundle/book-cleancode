...
1. [오류 코드보다 예외를 사용하라](#1)
1. [Try-Catch-Finally 문부터 작성하라](#2)
1. [미확인 예외를 사용하라](#3)
1. [예외에 의미를 제공하라](#4)
1. [호출자를 고려해 예외 클래스를 정의하라](#5)
1. [정상 흐름을 정의하라](#6)
1. [null을 반환하지 마라](#7)
1. [null을 전달하지 마라](#8)
1. [결론](#9)

## <a name = '1'> 오류 코드보다 예외를 사용하라 p.130 </a>
...
## <a name = '2'> Try-Catch-Finally문부터 작성하라 p.132 </a>
...
2. 선언부에 throw절을 추가해야한다.
```

- 아주 중요한 라이브러리를 작성한다면 모든 예외를 잡아야하는데 이럴 때는 확인된 예외가 유용하다.
- 하지만 일반적인 애플리케이션은 의존성이라는 비용이 이익보다 크다.

## <a name = '4'> 예외에 의미를 제공하라 p.135 </a>

- 예외를 던질 때는 전후 상황을 충분히 덧붙인다. 오류가 발생한 원인과 위치를 찾기 쉬워진다.
- 오류 메시지에 정보를 담아 예외와 함께 던진다. 실패한 연산 이름과 실패 유형도 언급한다.

## <a name = '5'> 호출자를 고려해 예외 클래스를 정의하라 p.135 </a>
...
## <a name = '6'> 정상 흐름을 정의하라 p.137 </a>

- 비즈니스 논리와 오류 처리가 잘 분리된 코드가 나온다면 오류 감지가 프로그램 언저리로 밀려난다.
...
## <a name = '7'> null을 반환하지 마라 p.138 </a>
...
## <a name = '8'> null을 전달하지 마라 p.140 </a>
